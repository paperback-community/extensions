...
var source = (() => {
  ...
  // src/Toonily/main.ts
  var DOMAIN_NAME = "https://toonily.com";
  ...
  var ToonilyExtension = class {
    ...
    async getNewOnToonily(metadata) {
      ...
      $2("section ul li.css-1urdgju").each((_, element) => {
      ...
      const nextButton = $2(".next_btn_topco-9MoRR_2");
      const hasNextPage = nextButton.length > 0 && !nextButton.attr("style")?.includes("display: none");
      return {
        items,
    ...
    async getLatestReleases(section, metadata) {
      ...
      const $2 = await this.fetchCheerio(request);
      $2(".page-listing-item .col-6.col-sm-3.col-lg-2").each((_, element) => {
        const unit = $2(element);
      ...
      const nextPageHref = $2(".nextpostslink").attr("href");
      let nextPage;
      if (nextPageHref) {
    ...
    async getTrendingSection(section, metadata) {
      const page = metadata?.page ?? 1;
      const items = [];
      const collectedIds = metadata?.collectedIds ?? [];
    ...
    async getMangaDetails(mangaId) {
      ...
      const altNames = [];
      $2('.summary-heading:contains("Alt Name(s)")').next(".summary-content").each((_, element) => {
        const altName = $2(element).text().trim();
        if (altName) altNames.push(altName);
      });
      const image = $2(".summary_image img").attr("data-src") || $2(".summary_image img").attr("src") || "";
      const validImage = image.trim();
      const description = $2(".summary__content p").text().trim();
      let status = "UNKNOWN";
      const statusText = $2('.summary-heading:contains("Status")').next(".summary-content").text().trim().toLowerCase();
      ...
      const authors = [];
      $2('.summary-heading:contains("Author(s)")').next(".summary-content").find("a").each((_, element) => {
        authors.push($2(element).text().trim());
      });
      const artists = [];
      $2('.summary-heading:contains("Artist(s)")').next(".summary-content").find("a").each((_, element) => {
        artists.push($2(element).text().trim());
      });
      const genres2 = [];
      $2('.summary-heading:contains("Genre(s)")').next(".summary-content").find("a").each((_, element) => {
        genres2.push($2(element).text().trim());
      });
      const tags = [];
      $2(".wp-manga-tags-list a").each((_, element) => {
    ...
    async getChapterDetails(chapter) {
      const url = new URLBuilder(DOMAIN_NAME).addPath("serie").addPath(chapter.sourceManga.mangaId).addPath(chapter.chapterId).build();
      const request = {
        url,
        method: "GET"
      };
      ...
      try {
        const $2 = await this.fetchCheerio(request);
        const pages = [];
        $2(".reading-content img.wp-manga-chapter-img").each((_, img) => {
          const rawSrc = ($2(img).attr("data-src") || $2(img).attr("src") || "").trim();
          if (rawSrc) {
            const src = rawSrc.startsWith("http") ? rawSrc : `${DOMAIN_NAME}${rawSrc}`;
