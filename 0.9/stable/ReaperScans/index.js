...
var source = (() => {
  ...
  // src/ReaperScans/main.ts
  var main_exports = {};
  __export(main_exports, {
    ReaperScans: () => ReaperScans,
    ReaperScansExtension: () => ReaperScansExtension
  });
  init_buffer();
  ...
  var RS_API_DOMAIN = "https://api.reaperscans.com";
  var RS_CDN_DOMAIN = "https://media.reaperscans.com/file/4SRBHm";
  ...
  // src/ReaperScans/ReaperParser.ts
  init_buffer();
  ...
  // src/ReaperScans/ReaperParser.ts
  var parseMangaDetails = (mangaDetails, mangaId) => {
    const $2 = load(mangaDetails.description);
    const description = $2("p").map((_, el) => $2(el).text().trim()).get().join(" ");
    return {
      mangaId,
      mangaInfo: {
        ...
        bannerUrl: mangaDetails.meta.background ?? void 0,
        rating: mangaDetails.rating / 5,
        tagGroups: parseTags(mangaDetails),
        shareUrl: new URLBuilder(RS_DOMAIN).addPath("series").addPath(mangaId).build()
      }
    };
  };
  var parseTags = (mangaDetails) => {
  ...
  var parseChapterList = (chapterList, sourceManga) => {
    const chapters = [];
    let sortingIndex = 0;
  ...
  var parseChapterDetails = (chapterDetails, chapter) => {
    const pages = [];
    if (chapterDetails.chapter?.storage == "local" && chapterDetails.chapter.chapter_data && "images" in chapterDetails.chapter.chapter_data) {
      for (const image of chapterDetails.chapter.chapter_data.images) {
        pages.push(checkImage(image));
      }
    } else if (chapterDetails.chapter.storage == "s3" && chapterDetails.chapter.chapter_data && "files" in chapterDetails.chapter.chapter_data) {
  ...
  var parseUpdatesSection = (queryResult) => {
    const items = [];
    for (const item of queryResult.data) {
  ...
  var parseDailySection = (queryResult) => {
    const items = [];
  ...
  // src/ReaperScans/main.ts
  var ReaperScansExtension = class {
    ...
    async getSearchResults(query, metadata) {
      ...
      const searchString = query.title?.trim().replace(/\s+/g, " ").replace(/ /g, "+");
      const request = {
        url: new URLBuilder2(RS_API_DOMAIN).addPath("query").addQuery("adult", "true").addQuery("query_string", searchString).addQuery("perPage", "200").addQuery("page", page).addQuery("series_type", "Comic").build(),
        method: "GET",
        headers: {
          "user-agent": await Application.getDefaultUserAgent(),
          referer: `${RS_DOMAIN}/`
    ...
    async getMangaDetails(mangaId) {
      const request = {
        url: new URLBuilder2(RS_API_DOMAIN).addPath("series").addPath(mangaId).build(),
        method: "GET",
        headers: {
          "user-agent": await Application.getDefaultUserAgent(),
          referer: `${RS_DOMAIN}/`
    ...
    async getChapters(sourceManga) {
      const request = {
        url: new URLBuilder2(RS_API_DOMAIN).addPath("chapters").addPath(sourceManga.mangaId).addQuery("perPage", 30).build(),
        method: "GET",
        headers: {
          "user-agent": await Application.getDefaultUserAgent(),
          referer: `${RS_DOMAIN}/`
      ...
      while (chapterListMeta.current_page < chapterListMeta.last_page) {
        const request2 = {
          url: new URLBuilder2(RS_API_DOMAIN).addPath("chapters").addPath(sourceManga.mangaId).addQuery("perPage", 30).addQuery("page", chapterListMeta.current_page + 1).build(),
          method: "GET",
          headers: {
            "user-agent": await Application.getDefaultUserAgent(),
            referer: `${RS_DOMAIN}/`
        ...
        const [_2, buffer2] = await Application.scheduleRequest(request2);
    ...
    async getChapterDetails(chapter) {
      const request = {
        url: new URLBuilder2(RS_API_DOMAIN).addPath("chapter").addPath(chapter.sourceManga.mangaId).addPath(chapter.chapterId).build(),
        method: "GET",
        headers: {
          "user-agent": await Application.getDefaultUserAgent(),
          referer: `${RS_DOMAIN}/`
    ...
    async getDiscoverSectionItems(section, metadata) {
      ...
      switch (section.type) {
        case import_types4.DiscoverSectionType.simpleCarousel:
          {
            ...
            metadata = page == json.meta.last_page ? void 0 : { page: page + 1 };
          }
          break;
        ...
        case import_types4.DiscoverSectionType.featured:
          {
            const request = {
              url: new URLBuilder2(RS_API_DOMAIN).addPath("trending").build(),
              method: "GET",
              headers: {
                "user-agent": await Application.getDefaultUserAgent(),
                referer: `${RS_DOMAIN}/`
  ...
  var ReaperScans = new ReaperScansExtension();
